...
public class GraphQLWebSocketConfiguration {
    ...
    open var connectionParams: () -> [String: AnyCodable]? = { nil }
    
    /// Controls when the client should establish a connection with the server.
    open var behaviour: Behaviour = .eager
    ...
    open var connectionAckTimeout: Int = 0
    
    /// Logger that we use to communitcate state changes inside the WebSocket.
    open var logger: Logger = Logger(label: "graphql.socket")
